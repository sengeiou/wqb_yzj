...
layui.define(["element", "jquery"], function (exports) {
  ...
  Tab.prototype.changeRegresh = function (index) {
    if (changeRefreshStr == "true") {
      ...
      // 判断是否跨域
      if ($iframe.src.indexOf(location.origin) > -1) {
  ...
  Tab.prototype.tabAdd = function (_this) {
    ...
    if (_this.attr("target") == "_blank") {
      ...
      if (that.hasTab(_this.find("cite").text()) == -1 && _this.siblings("dl.layui-nav-child").length == 0) {
        ...
        $('#top_tabs > li').each(function (index, el) {
          var title = $(this).children('cite').text();
          if (title === curmenu.title) {
            that.changeRegresh(index);
          }
        });
        // that.changeRegresh(_this.parent('.layui-nav-item').index()); // 左侧每个li的index不等于已打开的窗口index
        window.sessionStorage.setItem("curmenu", JSON.stringify(curmenu)); //当前的窗口
        element.tabChange(tabFilter, that.getLayId(_this.find("cite").text()));
        that.tabMove(); //顶部窗口是否可滚动
      }
  ...
  //切换后获取当前窗口的内容
  $("body").on("click", "#top_tabs li", function () {
    var curmenu = '',
      menu = JSON.parse(window.sessionStorage.getItem("menu")),
      $this = $(this);
    if (window.sessionStorage.getItem("menu")) {
  ...
  //删除tab
  $("body").on("click", "#top_tabs li i.layui-tab-close", function () {
  ...
  //刷新当前
  $("#top_tabs_box a.refresh").on("click", function () {
    ...
    if ($this.hasClass("refreshThis")) {
      ...
      // 判断是否跨域
      if ($iframe.src.indexOf(location.origin) >= 0) {
        $iframe.contentWindow.location.reload();
      } else { // 跨域
        $iframe.src = $iframe.src;
      }
      ...
      setTimeout(function () {
        // $("#top_tabs_box a.refresh").addClass("refreshThis");
